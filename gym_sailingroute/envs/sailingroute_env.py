...
def cardioid_r(a, phi, factor, speed_max=8):
    ...
    fun = 2 * a * (1 - np.cos(phi) + b * np.cos(phi * d /
                                                (2 * PI)) + c * np.sin(phi * e / (2 * PI)))
    phi_90 = phi < np.radians(90)
    phi_270 = phi > np.radians(270)
    phi_mid1 = (phi > np.radians(135))
    phi_mid2 = (phi < np.radians(225))
    ...
    fun = np.where(phi_mid1 & phi_mid2, fun - enhancing_f_2 *
                   np.sin(2 * (phi - 3 * PI / 4)), fun)
    return fun * factor
...
def generate_wind_field(maximum=100, n_steps=10, plotting_scale=1):
    ...
    # print(np.max(dx))
    skip = (slice(None, None, plotting_scale),
            slice(None, None, plotting_scale))
...
def goal_heading(start, goal):
    ...
    if norm == 0:
        return 0, 0  # set heading to goal to 0 if agent has reached the goal.
...
class SailingrouteEnv(gym.Env):
    ...
    def __init__(self, size=200, timestep=1, resolution=20):
        ...
        # this determines the resolution of the grid - which corresponds to the
        # wind observation!
        self.resolution = resolution
        # DO NOT CHANGE THIS! - 400 "pixel" for the NN (20x20)
        assert self.resolution == 20

        self.observation_space = spaces.Dict({"position": spaces.Box(low=0, high=self.size, shape=(2, 2)),
                                              # to be corrected for dict and
                                              # stuff
                                              "wind": spaces.Box(low=0, high=40, shape=(self.size, self.size)),
    ...
    def step(self, action):
        ...
        goal_head, goal_norm = goal_heading(
            self.state['position'][1], self.state['position'][0])
        # if np.sqrt(np.square(_dist[0]) + np.square(_dist[1]))  <=
        # self.threshold:
        ...
        vmg_reward = VMG(goal_head, action, speed) / self.boat_max_speed * \
            (step_punishment * 4) - step_punishment * 0.7
    ...
    def reset(self):
        ...
        windfield = generate_wind_field(
            n_steps=self.resolution, maximum=self.size)
        ...
        # needs to be updated in case of function and continuity
    ...
    def _plotting(self, x, y, dx, dy, tws, skip, x_curr, y_curr, goal, first, axi, done, **kwargs):
        ...
        if self.printing:
            plt.savefig('pictures/pic_{}.png'.format(now.isoformat()))
        plt.draw()
        plt.pause(0.01)
        return ax
    ...
    def render(self, mode='human', close=False):
        ...
        self.course_traversed.append(
            [self.state['position'][0, 0][0], self.state['position'][0, 1][0]])
        ...
        self.ax = self._plotting(x_curr=np.array(self.course_traversed)[:, 0],
                                 ...
                                 goal=goal, first=self.render_first, done=self._state[
                                     'done'],
                                 axi=self.ax,
                                 **self._state['wind'])
...
class SailingrouteEnv_simplest(gym.Env):
    ...
    def __init__(self, size=10, timestep=1, resolution=20):
        self.size = size  # this determines the physical size of the grid
        self.timestep = timestep  # this determines "how fast time evolves"
        # 1 corresponds to 1h/step, 2 corresponds to 0.5h/step and so on
        # this determines the resolution of the grid - which corresponds to the
        ...
        assert isinstance(self.resolution_action, int)
        self.action_space = spaces.Discrete(int(360 / self.resolution_action))
        self.reset()

        self.threshold = 1  # maximum distance to target to end the episode
        self.reward_range = (-1, 1)
        self.count = 0
    ...
    def step(self, action):
        ...
        goal_head, goal_norm = goal_heading(
            self._state['position'][1], self._state['position'][0])
        # if goal_norm <= self.threshold:
        #   self._state['done'] = True
        #   self.course_traversed = []
        #   return self.state, goal_reward, True, {'goal': 'reached'}
        ...
        # self.state['position'][np.where(self.state['position'] == 1)] = 0
        # self.state['position'][0][int(self._state['position'][1,0]), int(self._state['position'][1,1])] = 1 # start
        # self.state['position'][self._state['position'][0][0],self._state['position'][0][1]]
        # = 2 # goal

        # return self.state, -step_punishment+vmg_reward, False, {'some':
        # 'thing'}
        return self.state, vmg_reward, True, {'some': 'thing'}
        # return observation, reward, done, info
    ...
    def reset(self):
        ...
        self._state = {'position':  np.array([generate_random_point(
            self.size, pos=True), generate_random_point(self.size, pos=True)]), 'done': False}
        maxi_c = np.complex(0, self.resolution)
        x, y = np.mgrid[0:self.size:maxi_c, 0:self.size:maxi_c]
        self._state.update(dict(grid=[x, y]))

        # model position knowledge picture
        self.state = {'position': np.array(
            [np.zeros((self.size, self.size), dtype=np.int32)])}
        self.state['position'][0][int(self._state['position'][1, 0]), int(
            self._state['position'][1, 1])] = 1  # start
        self.state['position'][0][int(self._state['position'][0, 0]), int(
            self._state['position'][0, 1])] = 2  # goal
        # print(self.state['position'].shape)
        # print(self.state['position'])

        # np.max(self.boat) # needs to be updated in case of function and
        # continuity
        self.boat_max_speed = 1
        self.render_first = True
        self.course_traversed = []
        # self._state.update({'done': False})
        return self.state
    ...
    def _plotting(self, x, y, x_curr, y_curr, goal, first, axi, done, **kwargs):
        now = datetime.datetime.now()
        if first or done:
            if plt:
                plt.close()
    ...
    def render(self, mode='human', close=False):
        self.render_first = True if self._state['done'] else self.render_first
        goal = self._state['position'][1]
        self.course_traversed.append(
            [self._state['position'][0, 0][0], self._state['position'][0, 1][0]])
        # print(self.course_traversed[-1], goal)
        # print(self.ax, self.render_first, self._state['done'])
        ...
        self.ax = self._plotting(x_curr=np.array(self.course_traversed)[:, 0],
                                 ...
                                 goal=goal, first=self.render_first, done=self._state[
                                     'done'],
                                 axi=self.ax,
                                 x=self._state['grid'][0], y=self._state['grid'][1])
        self.render_first = False
